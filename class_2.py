...
# функция проверяет существует ли искомый столбец в заданном датафрейме
...
# функция устонавливает/преобразовывает тип данных существующего столбца данных в тип float
    try:
...
# agg - хранит число от 1 до 5, где 1 - среднее, 2 - медиана, 3 - мода, 4 - количество, 5 - количество уникальных значений
...
def scatter(data, column_1, column_2, column_outl=None):
...
# column_outl - колонка для отсеивания выбросов
    ...
    df = dropna(df)
    if column_outl:
        if column_outl == column_1 or column_outl == column_2:
            df = outlier(df, column_outl)
            df = df[[column_1, column_2]]
        else:
            print('Для обработки введены постронние данные. Выбросы не удалены')
    return df